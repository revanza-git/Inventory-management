...
@if($material == 'stock')
    <h2>Laporan Material Persediaan ({{ucwords($material)}}) {{cat($kategori)}} Periode {{Carbon\Carbon::parse($firstRange)->isoFormat('LL');}} - {{Carbon\Carbon::parse($secondRange)->isoFormat('LL');}}</h2>
@elseif($material == 'surplus')
    <h2>Laporan Material Projek / ({{ucwords($material)}}) Projek {{cat($kategori)}} Periode {{Carbon\Carbon::parse($firstRange)->isoFormat('LL');}} - {{Carbon\Carbon::parse($secondRange)->isoFormat('LL');}}</h2>
@elseif($material == 'dead')
    <h3>Laporan Material Persediaan Mati / ({{ucwords($material)}}) Stock Projek {{cat($kategori)}} Periode {{Carbon\Carbon::parse($firstRange)->isoFormat('LL');}} - {{Carbon\Carbon::parse($secondRange)->isoFormat('LL');}}</h3>
@elseif($material == 'rongsokan')
    <h3>Laporan Material {{ucwords($material)}} {{cat($kategori)}} Periode {{Carbon\Carbon::parse($firstRange)->isoFormat('LL');}} - {{Carbon\Carbon::parse($secondRange)->isoFormat('LL');}}</h3>
@else
