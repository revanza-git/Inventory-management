    ...
    <form action="{{ url('/revisionAllInHead') }}" method="post">
        ...
        <div class="mb-3 mt-3">
            ...
            <div class="form-floating mb-3">
                ...
                name="ReasonThirdApprovalDocsPartIn"></textarea>
            </div>
            <button
...
<div class="table-responsive-sm mt-4">
    <table
        ...
        <tbody>
            ...
            <tr>
                <td scope="row">
                    @if (empty($data->filePhotoPartIn) && empty($data->filePO) &&
                    empty($data->fileBAST))
                    <p>Data Dokumen Tidak ada</p>
                    @endif @if (!empty($data->filePhotoPartIn))
                    <a
                        type="button"
                        class="btn btn-sm greenAcc margin-button"
                        href="{{ asset('data/'.$data->filePhotoPartIn) }}">Foto
                        {{$data->namaPart}}</a>
